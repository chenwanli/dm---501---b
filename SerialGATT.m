...
{
    [self notification:fileService characteristicUUID:fileSub1 p:peripheral on:YES];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    BOOL exists = NO;
    for (CBPeripheral *peripheralInArray in peripherals) {
    ...
    if ((!exists))
    {
        [self.peripherals addObject:peripheral];// 搜索到新的设备添加到设备列表中并发送“发现新设备”的通知
        [delegate peripheralFound:peripheral];
    }
